...
gather_dep() {
    ...
    pkg=$(dpkg-query --search "$source" | cut -d: -f1)
    ...
    if copyright=$(dpkg-query --listfiles "$pkg" | grep -E "/copyright$"); then
        echo "installing deb package $pkg copyright file $copyright"
